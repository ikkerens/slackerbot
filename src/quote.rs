...
pub(crate) async fn post_quote<'a>(
	ctx: &Context, quote: quote::Model, channel: ChannelId, response: Option<ApplicationCommandInteraction>,
) -> Result<()> {
	let (avatar_url, avatar_data) = if let Some(author_image) = quote.author_image {
		(
			Some("attachment://avatar.png".to_string()),
			Some(convert_bytes_to_attachment("avatar.png", author_image)),
		)
	} else {
